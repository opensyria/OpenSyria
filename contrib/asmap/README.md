...
be used in OpenSyria Core with the `-asmap` option.
...
to binary before being used with OpenSyria Core.
...
the `opensyria-cli getnodeaddresses` command.
...
```
opensyria-cli getnodeaddresses 0 > addrs.json
```
