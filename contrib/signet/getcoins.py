...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing opensyria-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='opensyria-cli', help='opensyria-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='OpenSyria address to which the faucet should send')
...
parser.add_argument('opensyria_cli_args', nargs='*', help='Arguments to pass on to opensyria-cli (default: -signet)')

args = parser.parse_args()

if args.opensyria_cli_args == []:
    args.opensyria_cli_args = ['-signet']


def opensyria_cli(rpc_command_and_params):
    argv = [args.cmd] + args.opensyria_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = opensyria_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = opensyria_cli(['getnewaddress', 'faucet', 'bech32'])
