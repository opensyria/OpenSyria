...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing opensy-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='opensy-cli', help='opensy-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='OpenSY address to which the faucet should send')
...
parser.add_argument('opensy_cli_args', nargs='*', help='Arguments to pass on to opensy-cli (default: -signet)')

args = parser.parse_args()

if args.opensy_cli_args == []:
    args.opensy_cli_args = ['-signet']


def opensy_cli(rpc_command_and_params):
    argv = [args.cmd] + args.opensy_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = opensy_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = opensy_cli(['getnewaddress', 'faucet', 'bech32'])
