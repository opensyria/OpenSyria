...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/opensy-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our OpenSY build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built OpenSY to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example opensy.conf file. if contrib/devtools/gen-opensy.conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/opensy.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/opensy-macos-app.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
