...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/opensyria-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our OpenSyria Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built OpenSyria Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example opensyria.conf file. if contrib/devtools/gen-opensyria.conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/opensyria.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/opensyria-macos-app.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
