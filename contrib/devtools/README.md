...
Provides utilities for managing copyright headers of `The OpenSY
...
---------------------------------------------------------
Updates all the copyright headers of `The OpenSY developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The OpenSY developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The OpenSY developers
```
...
those cases:
```
// Copyright (c) <year> The OpenSY developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The OpenSY developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The OpenSY developers` at the top of the
...
If the file already has a copyright for `The OpenSY developers`, the
script will exit.
...
Generates a opensy.conf file in `share/examples/` by parsing the output from `opensyd --help`. This script is run during the
release process to include a opensy.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-opensy.conf.sh
```
