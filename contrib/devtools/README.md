...
Provides utilities for managing copyright headers of `The OpenSyria Core
...
---------------------------------------------------------
Updates all the copyright headers of `The OpenSyria Core developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The OpenSyria Core developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The OpenSyria Core developers
```
...
those cases:
```
// Copyright (c) <year> The OpenSyria Core developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The OpenSyria Core developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The OpenSyria Core developers` at the top of the
...
If the file already has a copyright for `The OpenSyria Core developers`, the
script will exit.
...
gen-opensyria.conf.sh
===================

Generates a opensyria.conf file in `share/examples/` by parsing the output from `opensyriad --help`. This script is run during the
release process to include a opensyria.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-opensyria.conf.sh
```
