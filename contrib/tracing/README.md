...
Tracing (USDT) support for OpenSY on Linux using. For more information on
USDT support in OpenSY see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `opensyd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `opensyd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[OpenSY USDT documentation] on how to list available tracepoints in your
`opensyd` binary.

[OpenSY USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof opensyd)
```
...
OpenSY P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof opensyd)
```
...
In a different terminal, starting OpenSY in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/opensyd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof opensyd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof opensyd)
```
