...
Tracing (USDT) support for OpenSyria Core on Linux using. For more information on
USDT support in OpenSyria Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `opensyriad` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `opensyriad` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[OpenSyria Core USDT documentation] on how to list available tracepoints in your
`opensyriad` binary.

[OpenSyria Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof opensyriad)
```
...
OpenSyria P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof opensyriad)
```
...
In a different terminal, starting OpenSyria Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/opensyriad -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof opensyriad)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof opensyriad)
```
