OpenSyria Core version 0.18.0 is now available from:

  <https://opensyriacore.org/bin/opensyria-core-0.18.0/>
...
`/Applications/OpenSyria-Qt` (on Mac) or `opensyriad`/`opensyria-qt` (on
Linux).
...
recommended to use OpenSyria Core on unsupported systems.
...
10.10. Additionally, OpenSyria Core does not yet change appearance when
...
`contrib/init/opensyriad.service`, it has been changed to use
`/var/lib/opensyriad` as the data directory instead of
`~opensyria/.opensyria`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/opensyriad` will
exist has enough space (check using `df -h /var/lib/opensyriad`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/opensyria/opensyria/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://opensyriacore.org/en/contact/) the OpenSyria Core
...
The systemd init file (`contrib/init/opensyriad.service`) has been changed
to use `/var/lib/opensyriad` as the data directory instead of
`~opensyria/.opensyria`. This change makes OpenSyria Core more consistent with
...
`datadir` option in `/etc/opensyria/opensyria.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/opensyria/opensyria.conf`.


Documentation
-------------

- A new short [document](https://github.com/opensyria/opensyria/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/opensyria/opensyria/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/opensyria/opensyria/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/opensyria/opensyria/blob/master/doc/opensyria-conf.md)
  about the `opensyria.conf` file describes how to use it to configure
  OpenSyria Core.

- A new document introduces OpenSyria Core's BIP174 [Partially-Signed
  OpenSyria Transactions
  (PSBT)](https://github.com/opensyria/opensyria/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/opensyria/opensyria/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent OpenSyria-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  OpenSyria Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/opensyria/opensyria/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/opensyria/opensyria/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when OpenSyria Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of OpenSyria Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/opensyria/opensyria/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/opensyria/opensyria/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart opensyriad with the `-deprecatedrpc=generate`
...
- A new `opensyria-wallet` tool is now distributed alongside OpenSyria
...
This section describes planned changes to OpenSyria Core that may affect
other OpenSyria software and services.

- Since version 0.16.0, OpenSyria Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with OpenSyria Core 0.20 (expected about
  a year after 0.18), OpenSyria Core will default to native segwit
  ...
  to bech32 addresses, and if the OpenSyria Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in OpenSyria Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any OpenSyria Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, OpenSyria Core
...
- A sub-project of OpenSyria Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with OpenSyria Core.  See their [project
  page](https://github.com/opensyria-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to OpenSyria Core's own implementation, although entropy
  gathered by OpenSyria Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves OpenSyria Core a
...
- On macOS, OpenSyria Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `opensyria_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libopensyria_server.a` (jnewbery)
...
- #13926 [Tools] opensyria-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like opensyriad (MarcoFalke)
...
- #14521 qt, docs: Fix `opensyria-qt -version` output formatting (hebasto)
...
- #11625 Add OpenSyriaApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libopensyria_server` (laanwj)
...
- #15154 Configure: opensyria-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/opensyria-wallet (Sjors)
...
- #11526 Visual Studio build configuration for OpenSyria Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libopensyria_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in opensyria-util-test.py (isghe)
...
- #12775 Integration of property based testing into OpenSyria Core (Christewart)
...
- #12246 Bugfix: Only run opensyria-tx tests when opensyria-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_opensyria` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_opensyria` (MarcoFalke)
...
- #14186 opensyria-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_opensyria.conf` message on startup if conf file exists (leishman)
...
- #12255 Update opensyria.service to conform to init.md (dongcarl)
...
- #14497 Add doc/opensyria-conf.md (hebasto)
...
- #15354 Add missing `opensyria-wallet` tool manpages (MarcoFalke)
...
- #15765 correct opensyriaconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support opensyria folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/opensyria/).
