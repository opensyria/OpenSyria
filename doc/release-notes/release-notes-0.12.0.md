OpenSyria Core version 0.12.0 is now available from:

  <https://opensyria.org/bin/opensyria-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of OpenSyria Core or other software:
...
pre-0.12 versions of OpenSyria Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start OpenSyria Core version 0.11 or
earlier.
...
ECDSA signatures inside OpenSyria transactions now use validation using
[libsecp256k1](https://github.com/opensyria-core/secp256k1) instead of OpenSSL.
...
Previous versions of OpenSyria Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of OpenSyria Core was to raise the default minimum
relay fee.

OpenSyria Core 0.12 will have a strict maximum size on the mempool. The
...
OpenSyria Core 0.12 also introduces new default policy limits on the length and
...
OpenSyria Core 0.12 nodes. OpenSyria Core will only allow replacement of
...
Note that the wallet in OpenSyria Core 0.12 does not yet have support for
...
This allows running opensyriad without having to do any manual configuration.
...
OpenSyria Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. OpenSyria Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In OpenSyria Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in OpenSyria Core 0.13, and it is at this time undecided whether the
...
OpenSyria Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
OpenSyria Core automatically creates a hidden service to listen on, without
manual configuration. OpenSyria Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if OpenSyria Core is listening, and
...
OpenSyriad can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals OpenSyria
Core to use floating fees. By default, OpenSyria Core will use floating
fees.
...
At all times, OpenSyria Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SYL.
Furthermore, OpenSyria Core will never create transactions paying less than
...
`opensyria.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/opensyria/opensyria/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
caching. A sample config for apache2 could look like:
    ...
    <Location /opensyriarpc>
        ...
        # optional bypass opensyriad rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< opensyriarpc:<password>
    </Location>
...
Restarting opensyriad will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of OpenSyria-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new opensyria-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include opensyria-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix opensyria-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of opensyria-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override OPENSYRIAD and OPENSYRIACLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [opensyria-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add OPENSYRIAD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libopensyriaconsensus (Braydon Fuller)
