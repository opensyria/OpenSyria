...
OpenSyria Core version 0.20.0 is now available from:

  <https://opensyriacore.org/bin/opensyria-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `OPENSYRIA_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from OpenSyria Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since OpenSyria Core version 0.18.0.
...
- Testing or debugging of implementations of the OpenSyria P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of OpenSyria Core. OpenSyria Core logs debug messages
...
- OpenSSL is no longer used by OpenSyria Core.  (#17265)

- BIP70 support has been fully removed from OpenSyria Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/opensyria/opensyria/issues/16599),
  [PR #16702](https://github.com/opensyria/opensyria/pull/16702), and the
  `opensyriad help` for more information.  This option is experimental and
...
- All custom settings configured when OpenSyria Core starts are now
...
- The "Start OpenSyria Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://opensyriacore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple OpenSyria Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed OpenSyria Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/opensyria-core/HWI) for
  signing.  Future versions of OpenSyria Core should support a GUI option
...
- OpenSyria Core's automatically-generated source code documentation is
  now available at https://doxygen.opensyriacore.org.  (#17596)
...
- The `opensyria-cli` utility used with the `-getinfo` parameter now
...
- The `opensyria-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_opensyria-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBOPENSYRIA_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_opensyria` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBOPENSYRIA_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for opensyria-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate opensyria schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest opensyria.conf (sdaftuar)
...
- #18022 Fix appveyor `test_opensyria` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (opensyria-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone opensyria-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about OpenSyria Core files (hebasto)
...
- #17119 Fix broken opensyria-cli examples (andrewtoth)
...
- #17503 Remove bitness from opensyria-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.opensyriacore.org (laanwj)
...
- #17648 Rename wallet-tool references to opensyria-wallet (hel-o)
...
- #16115 On opensyriad startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from opensyria-cli -getinfo (malevolent)
