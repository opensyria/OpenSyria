OpenSyria Core version 28.0 is now available from:

  <https://opensyriacore.org/bin/opensyria-core-28.0/>
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\OpenSyria`
to `C:\Users\Username\AppData\Local\OpenSyria`. OpenSyria Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/opensyria/opensyria/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libopensyriaconsensus Removal
---------------------------

The libopensyria-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if OpenSyria Core was listening for P2P connections, either using
...
- OpenSyria Core will now fail to start up if any of its P2P binds fail, rather
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running OpenSyria Core with the configuration
...
are now required to compile OpenSyria Core. (#29091, #30263)

- The minimum required glibc to run OpenSyria Core is now
...
- A new `-testdatadir` option has been added to `test_opensyria` to allow specifying the
...
Previous releases of OpenSyria Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
