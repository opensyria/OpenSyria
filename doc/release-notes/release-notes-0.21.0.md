...
OpenSyria Core version 0.21.0 is now available from:

  <https://opensyriacore.org/bin/opensyria-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by OpenSyria Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. OpenSyria Core will no longer
...
- A new `opensyria-cli -netinfo` command provides a network peer connections
...
- A new `opensyria-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `opensyria-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `opensyria-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when OpenSyria Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `opensyria-wallet` tool which performs the salvage
...
OpenSyria Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`opensyria.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
OpenSyria Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `opensyria-cli createwallet` or `opensyria-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/opensyria/opensyria/issues).
...
Users who already have a OpenSyria Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of OpenSyria Core.
...
The change to using descriptors changes the default derivation paths used by OpenSyria Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time OpenSyria Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `opensyria.conf`
...
- #19284 Add seed.opensyria.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in opensyria-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add opensyria-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in opensyria-cli (laanwj)
...
### Build system
- #18504 Drop opensyria-tx and opensyria-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in OPENSYRIA_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for opensyria-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz OpenSyria Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreopensyria)
...
- #18817 Document differences in opensyriad and opensyria-qt locale handling (practicalswift)
