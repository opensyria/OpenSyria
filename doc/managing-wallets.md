...
Since version 0.21, OpenSyria Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `opensyria-cli help createwallet`.

```
$ opensyria-cli createwallet "wallet-01"
```
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.opensyria/wallets`                             |
| Windows          | `C:\Users\<user>\AppData\Local\OpenSyria\wallets`             |
| macOS            | `/Users/<user>/Library/Application Support/OpenSyria/wallets` |
...
```
$ opensyria-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ opensyria-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ opensyria-cli -named createwallet wallet_name="wallet-01" passphrase="passphrase"
```
...
```
$ opensyria-cli -rpcwallet="wallet-01" sendtoaddress "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx" 0.01
error code: -13
error message:
...
```
$ opensyria-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ opensyria-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
```
$ opensyria-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ opensyria-cli -rpcwallet="restored-wallet" getwalletinfo
```
...
Understanding wallet security is crucial for safely storing your OpenSyria. A key aspect is the wallet passphrase, used for encryption. Let's explore its nuances, role, encryption process, and limitations.
