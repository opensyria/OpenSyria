...
This guide describes how to build opensyriad, command-line utilities, and GUI on FreeBSD.
...
To build OpenSyria Core without the wallet, use `-DENABLE_WALLET=OFF`.
...
### 2. Clone OpenSyria Repo
Now that `git` and all the required dependencies are installed, let's clone the OpenSyria Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
OpenSyria Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
