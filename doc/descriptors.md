# Support for Output Descriptors in OpenSyria Core

Since OpenSyria Core v0.17, there is support for Output Descriptors. This is a
...
OpenSyria Core v24 extended `wsh()` output descriptor with [Miniscript](https://opensyria.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25. And since v26 Miniscript expressions can now be used in Taproot descriptors.
...
- [Miniscript](https://opensyria.sipa.be/miniscript/) expressions in `wsh` (P2WSH) and `tr` (P2TR) functions.
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.opensyria.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
...
on OpenSyria's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a OpenSyria Core descriptor wallet to serve as your
...
existing OpenSyria Core wallets, a convenience function `combo` is
...
All RPCs in OpenSyria Core will include the checksum in their output. Only
