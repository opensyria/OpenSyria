# CJDNS support in OpenSyria Core

It is possible to run OpenSyria Core over CJDNS, an encrypted IPv6 network that
...
done outside of OpenSyria Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the OpenSyria network and individual nodes.
...
before it will work with your OpenSyria Core node. This is described in steps
...
## Run OpenSyria Core with CJDNS

Once you are connected to the CJDNS network, the following OpenSyria Core
...
When enabled, this option tells OpenSyria Core that it is running in an
...
IPv6 local network. This helps OpenSyria Core perform better address management:
...
CJDNS support was added to OpenSyria Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `opensyria-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/opensyria/opensyria/blob/master/doc/tor.md) for
details.

## CJDNS-related information in OpenSyria Core

There are several ways to see your CJDNS address in OpenSyria Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `opensyria-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `opensyria-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `opensyria-cli help getnodeaddresses` for details.
