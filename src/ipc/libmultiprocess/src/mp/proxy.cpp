...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking opensyria code so they have to run
    ...
    // functions run, so client objects are fully disconnected before opensyria
    // code in the destructors are run. This way if the opensyria code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In opensyria core, running these callbacks in the right order is
