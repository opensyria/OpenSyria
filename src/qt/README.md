This directory contains the source code for the OpenSyria Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the OpenSyria UI.
...
- ClientModel is used by the main application `opensyriagui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following an opensyria:... link from a browser).
...
* UI elements like OpenSyriaAmountField, which inherit from QWidget.
* `opensyriastrings.cpp`: automatically generated
* `opensyriaunits.(h/cpp)`: SYL / mSYL / etc. handling
* `callback.h`
...
**Note:** Do not change `local/opensyria_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `opensyria-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `opensyria-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `opensyria-qt`
9. You're all set! Start developing, building, and debugging the OpenSyria Core GUI
