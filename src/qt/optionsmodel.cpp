...
/** Call node.updateRwSetting() with OpenSyria 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because OpenSyria 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/OpenSyria/OpenSyria/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because opensyria 22.x will not try to read these.
...
//! Convert enabled/size values to opensyria -prune setting.
...
//! Get pruning enabled value to show in GUI from opensyria -prune setting.
...
//! Get pruning size value to show in GUI from opensyria -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayOpenSyriaUnit")) {
        settings.setValue("DisplayOpenSyriaUnit", QVariant::fromValue(OpenSyriaUnit::SYL));
    }
    QVariant unit = settings.value("DisplayOpenSyriaUnit");
    if (unit.canConvert<OpenSyriaUnit>()) {
        m_display_opensyria_unit = unit.value<OpenSyriaUnit>();
    } else {
        m_display_opensyria_unit = OpenSyriaUnit::SYL;
        settings.setValue("DisplayOpenSyriaUnit", QVariant::fromValue(m_display_opensyria_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if opensyria.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_opensyria_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OpenSyriaUnit>() == m_display_opensyria_unit) return;
    m_display_opensyria_unit = new_unit.value<OpenSyriaUnit>();
    QSettings settings;
    settings.setValue("DisplayOpenSyriaUnit", QVariant::fromValue(m_display_opensyria_unit));
    Q_EMIT displayUnitChanged(m_display_opensyria_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/OpenSyria/OpenSyria/pull/8273
    ...
    // (https://github.com/opensyria-core/gui/issues/567).
