...
# TestP2PConn: A peer we use to send messages to opensyriad, and store responses.
...
class CompactBlocksTest(OpenSyriaTestFramework):
    ...
    # This test actually causes opensyriad to (reasonably!) disconnect us, so do this last.
    ...
    # Test that opensyriad requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for opensyriad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # opensyriad will not send blocktxn responses for blocks whose height is
    ...
    def test_low_work_compactblocks(self, test_node):
        ...
        node = self.nodes[0]
        hashPrevBlock = int(node.getblockhash(node.getblockcount() - 30000), 16)
        block = self.build_block_on_tip(node)
