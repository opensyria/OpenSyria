...
class ToolWalletTest(OpenSyriaTestFramework):
    ...
    def opensyria_wallet_process(self, *args):
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.opensyria_wallet_process(*args)
    ...
    def write_dump(self, dump, filename, magic=None, skip_checksum=False):
        if magic is None:
            magic = "OPENSYRIA_CORE_WALLET_DUMP"
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `opensyria-wallet help` raises an error. Use `opensyria-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `opensyria-wallet -help` for valid methods.')
    ...
    def test_dump_createfromdump(self):
        ...
        assert_equal(dump_data['OPENSYRIA_CORE_WALLET_DUMP'], '1')
        ...
        dump_data["OPENSYRIA_CORE_WALLET_DUMP"] = "0"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of opensyria-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["OPENSYRIA_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of opensyria-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_magic_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_magic.dump"
        del dump_data["OPENSYRIA_CORE_WALLET_DUMP"]
        dump_data["not_the_right_magic"] = "1"
        self.write_dump(dump_data, bad_magic_wallet_dump, "not_the_right_magic")
        self.assert_raises_tool_error('Error: Dumpfile identifier record is incorrect. Got "not_the_right_magic", expected "OPENSYRIA_CORE_WALLET_DUMP".', '-wallet=badload', '-dumpfile={}'.format(bad_magic_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
